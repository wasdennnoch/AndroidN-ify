...
public class BatteryInfo {
    ...
    public void bindHistory(final UsageView view, BatteryDataParser... parsers) {
        ...
        String timeString = res.getString(R.string.charge_length_format,
                XposedHelpers.callStaticMethod(Formatter.class, "formatShortElapsedTime", new Class[] {Context.class, long.class}, context, timePeriod));
        String remaining = "";
        if (remainingTimeUs != 0) {
            remaining = res.getString(R.string.remaining_length_format,
                    XposedHelpers.callStaticMethod(Formatter.class, "formatShortElapsedTime", new Class[] {Context.class, long.class}, context, remainingTimeUs / 1000));
        }
    ...
    public static BatteryInfo getBatteryInfo(Context context, Intent batteryBroadcast,
        ...
        final Resources resources = ResourceUtils.getInstance(context).getResources();
        if (!info.mCharging) {
            final long drainTime = stats.computeBatteryTimeRemaining(elapsedRealtimeUs);
            if (drainTime > 0) {
                info.remainingTimeUs = drainTime;
                String timeString = (String) XposedHelpers.callStaticMethod(Formatter.class, "formatShortElapsedTime", new Class[] {Context.class, long.class}, context,
                        drainTime / 1000);
